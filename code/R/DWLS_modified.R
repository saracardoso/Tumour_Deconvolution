...
if (!require("MAST")) {
  if(!require("BiocManager")) install.packages("BiocManager", dependencies = TRUE, repos="http://cran.r-project.org")
...
DEAnalysis<-function(scdata,id){
  exprObj<-Seurat::CreateSeuratObject(as.data.frame(scdata), project = "DE")
  exprObj2<-Seurat::SetIdent(exprObj,value=as.vector(id))
  print("Calculating differentially expressed genes:")
  res = list()
  for (i in unique(id)){
    res[[i]] <- Seurat::FindMarkers(object=exprObj2, ident.1 = i, ident.2 = NULL, 
...
buildSignatureMatrixUsingSeurat<-function(scdata,id,diff.cutoff=0.5,pval.cutoff=0.01){
  
  #perform differential expression analysis
  de_analysis = DEAnalysis(scdata,id)
  
  cluster_lrTest.tables = list()
  numberofGenes<-c()
  for (i in unique(id)){
    de_group = de_analysis[[i]]
    ...
    cluster_lrTest.tables[[paste("cluster_lrTest.table.",i,sep="")]] = de_group[which(rownames(de_group)%in%DEGenes[nonMir]),]
  ...
  for(G in 50:200){
    ...
    for (i in unique(id)){
      if(numberofGenes[j]>0){
        temp<-cluster_lrTest.tables[[paste("cluster_lrTest.table.",i,sep="")]]#paste("cluster_lrTest.table.",i,sep="")
        #temp<-as.name(temp)
...
DEAnalysisMAST<-function(scdata,id){
  ...
  for (i in unique(id)){
    ...
    if(dim(DE)[1]>1){
      ...
      show(zlm.output)
      coefAndCI <- MAST::summary(zlm.output, logFC=TRUE)
      ...
      #. 4 save results
      res[[i]] = cluster_lrTest.table
    }
  }
}

#build signature matrix using genes identified by DEAnalysisMAST()
buildSignatureMatrixMAST<-function(scdata,id,diff.cutoff=0.5,pval.cutoff=0.01){
  #compute differentially expressed genes for each cell type
  de_analysis = DEAnalysisMAST(scdata,id)
  ...
  cluster_lrTest.tables = list()
  for (i in unique(id)){
    if(i%in%names(de_analysis)){
      cluster_lrTest.table = de_analysis[[i]]
      ...
      cluster_lrTest.tables[[paste("cluster_lrTest.table.",i,sep="")]] = cluster_lrTest.table[which(cluster_lrTest.table$Gene%in%DEGenes[nonMir]),]
  ...
  for(G in 50:200){
    ...
    for (i in unique(id)){
      if(numberofGenes[j]>0){
        temp<-cluster_lrTest.tables[[paste("cluster_lrTest.table.",i,sep="")]]#temp<-paste("cluster_lrTest.table.",i,sep="")
        #temp<-as.name(temp)
        #temp<-eval(parse(text = temp))
